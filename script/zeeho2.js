...
class UserInfo {
  ...
  async createArticle() {
    try {
      ...
      if (res?.code == '10000' && res?.message == '操作成功') {
        $.log(`✅ 创建动态: 成功`);
        // 尝试从响应中提取动态 ID（若存在）
        this.lastPostId = res?.data?.tuuid || res?.data?.articleId || res?.data?.postId || null;
        if (this.lastPostId) {
          $.log(`✅ 创建动态返回 ID: ${this.lastPostId}`);
        }
      }
    } catch (e) {
      this.ckStatus = false;
  ...
  // 获取动态列表（增加重试机制，优先使用 lastPostId）
  async getArticles() {
    // 如果之前 createArticle 已返回 ID，直接使用
    if (this.lastPostId) {
      $.log(`✅ 使用创建动态返回的 ID: ${this.lastPostId}`);
    ...
    try {
      ...
      if (res?.code == '10000' && res?.message == '操作成功') {
        ...
        // 若未取到有效 ID，等待后重试（最多 3 次）
        for (let i = 0; i < 3; i++) {
          $.log(`⚠️ 动态列表为空或缺少 tuuid，${2 * (i + 1)} 秒后重试...`);
          await $.wait(2000 * (i + 1));
          res = await this.fetch(opts);
          if (res?.code == '10000' && Array.isArray(res?.data) && res?.data.length > 0) {
            const postId = res?.data[0]?.tuuid;
            if (postId) {
              $.log(`✅ 重试获取动态成功: ${postId}`);
              return postId;
            }
