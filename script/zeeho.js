  ...
  try {
    ...
    for (let user of userList) {
      ...
      if (user.ckStatus) {
        ...
        const { count = 0, prize = 0, prizes = 0 } = await user.getSignRecord() || {}
        await $.wait(user.getRandomTime());
        if(prizes >= 30) {
...
class UserInfo {
    ...
    try {
      ...
      if (res?.code == '10000' && res?.message == '操作成功') {
        ...
        for (let i = todayIndex; i >= 0; i--) {
          ...
          if (status == 3 || status == 5) count++;
          else break;
        }
        const prize = res?.data?.integral || 0;
        const prizes = res?.data?.signCount || 0;
        $.log(`✅ 连续签到${count}天 | 今日积分${prize} | 连签奖励累计${prizes}`);
  ...
  async lottery() {
    try {
      ...
      const params = { supplementDate: today }
      const opts = {
  ...
  async getCommunityArticle() {
    try {
      const opts = {
        ...
        headers: Object.assign({}, this.headers, getSign('app')),
        dataType: "json",
        params: { page: 1, pageSize: 20, postModule: 2, slidingType: 1 }
      }
...
async function getCookie() {
  ...
  const newData = { "userId": id, "token": token, "userName": nickName, "userAgent": userAgent }
...
async function Request(o) {
  ...
  try {
    ...
    return Promise.race([new Promise((_, e) => setTimeout(() => e('当前请求已超时'), timeout)), httpPromise]);
  } catch (e) {
    console.log(`❌请求发起失败！原因为：${e}`);
  }
};
function randomInt(n, r) { return Math.round(Math.random() * (r - n) + n) };
function DoubleLog(data) {
  if (data && $.isNode()) { console.log(`${data}`); $.notifyMsg.push(`${data}`) }
  else if (data) { console.log(`${data}`); $.notifyMsg.push(`${data}`) }
};
...
async function SendMsgList(l) {
  const msg = [...(l || []).map(u => u?.message?.join('\n')).filter(Boolean), ...($.notifyMsg || []).filter(Boolean)].join('\n');
  await SendMsg(msg);
};
async function SendMsg(n, o) {
  n && (0 < Notify ? $.isNode() ? await notify.sendNotify($.name, n) : $.msg($.name, $.title || "", n, { "media-url": o }) : console.log(n))
};
...
.finally(async () => { await SendMsgList($.notifyList); $.done({ ok: 1 }); });
